...
O contrato `supplier_return_fiscal_preparation_v1` faz somente o diagnóstico documental de uma entrada de compra finalizada. A tela da entrada mostra as evidências disponíveis e as pendências, mas não cria NF-e, não reserva série ou número, não escolhe CFOP ou tributação, não movimenta estoque e não chama Focus ou SEFAZ direta.
...
- [ ] Criar rascunho próprio e imutavelmente ligado à entrada e à chave original.
- [ ] Selecionar itens e quantidades, validando o limite recebido e eventual devolução anterior.

...
Na Central do servidor, o painel exclusivo do Master oferece três passos sem exigir digitação de comandos: seleção da filial para baixar a prévia; preenchimento dos cinco IDs, dos dois responsáveis e das observações opcionais para baixar a ficha; e geração do relatório final v3. A ficha exige confirmação de que os IDs foram escolhidos manualmente. O relatório exige nova confirmação e escolha obrigatória entre `sinteticos` e `reais`; somente a opção real aplica a trava histórica ao aceite. A interface não emite documento fiscal, não transmite dados e não registra aprovação.

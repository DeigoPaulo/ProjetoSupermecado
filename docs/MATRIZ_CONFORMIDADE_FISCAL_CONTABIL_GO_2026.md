...
- [ ] Criar memória de cálculo não emissiva por item, usando somente valores expressamente informados pelo responsável fiscal.
- [ ] Revisar a Fase 1 com o contador quando os dados reais chegarem.

## Próxima ação verificável

O fluxo já preserva origem, itens, `nItem`, snapshot e hash do XML. A revisão aprova somente a preparação, e o parecer registra a orientação global. A ficha por item exige escolha explícita do parecer, classifica todos os itens atomicamente e preserva origem e CST/CSOSN do ICMS, CST ou `NA` de IPI/PIS/COFINS, código cBenef GO ou `NA` e orientações complementares sem copiar a entrada. A próxima ação interna é criar memória de cálculo não emissiva por item, recebendo bases, alíquotas e valores somente quando informados pelo responsável e conferindo totalizações. XML, numeração e transmissão continuam bloqueados. Quando os dados reais estiverem disponíveis, ainda será necessário o aceite contábil e a homologação por canal.

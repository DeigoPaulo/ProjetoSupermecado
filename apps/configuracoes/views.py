...
CHECKLIST_GRUPOS = [
    ...
    },
    {
        ...
        "itens": [
            ...
            ("Devolução fiscal ao fornecedor", "partial", "O fluxo não emissivo confere entrada/XML, preserva chave, nItem e snapshot, limita quantidades e submete com SHA-256. A fila exclusiva de Administrador/Contabilidade permite corrigir ou aprovar somente a preparação; Compras e Financeiro não decidem. O parecer append-only registra natureza, regime, CFOP oficial e orientação humana. A ficha `supplier_return_item_tax_parameters_v1` exige escolha explícita do parecer e classifica atomicamente todos os nItem com origem e CST/CSOSN do ICMS, CST ou NA de IPI/PIS/COFINS, código cBenef GO ou NA e orientações complementares. A memória `supplier_return_item_tax_calculation_memory_v1` recebe bases, alíquotas e valores informados para todos os itens, exige zeros quando não aplicáveis e confere cada total antes de gravar. Versões idempotentes preservam parecer, ficha, XML e conteúdo por hash, sem copiar a entrada, definir fórmula ou alimentar XML. Nada cria documento, numeração, estoque ou Focus/SEFAZ. Próxima etapa: revisão segregada da memória completa, ainda sem liberar emissão."),

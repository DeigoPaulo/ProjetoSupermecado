...
class PreparacaoDevolucaoFornecedorTests(TestCase):
    ...
    def test_extracao_produtos_usa_somente_memoria_aprovada_sem_recalcular(self):
        ...
        self.assertEqual(grupos["pis"]["variante_candidata"], "")
        self.assertEqual(grupos["pis"]["modalidade_calculo_candidata"], "")
        self.assertEqual(grupos["pis"]["variante_fonte"], "DECISAO_CONTADOR_PENDENTE")
        self.assertEqual(grupos["pis"]["estado_variante"], "NAO_DEFINIDA")
        self.assertEqual(grupos["cofins"]["variante_candidata"], "")
        self.assertEqual(grupos["cofins"]["modalidade_calculo_candidata"], "")
        self.assertFalse(tributos["validacao"]["permite_aplicar_variantes_pis_cofins"])
        ...
        ipi_devolvido = extrair_contrato_devolucao(rascunho.pk, self.revisor)["ipi_devolvido"]
        self.assertTrue(ipi_devolvido["validacao"]["estrutura_valida"])
        self.assertTrue(ipi_devolvido["validacao"]["origem_completa"])
        self.assertEqual(ipi_devolvido["conteudo"]["itens"][0]["ipi_memoria"]["valor"], "0.00")
        self.assertEqual(ipi_devolvido["conteudo"]["itens"][0]["enquadramento_ipi"]["valor_candidato"], "")
        self.assertEqual(ipi_devolvido["conteudo"]["itens"][0]["enquadramento_ipi"]["fonte"], "DECISAO_CONTADOR_PENDENTE")
        self.assertFalse(ipi_devolvido["validacao"]["enquadramento_ipi_definido"])
        self.assertEqual(ipi_devolvido["conteudo"]["itens"][0]["imposto_devol"]["pdevol"], "")
        self.assertEqual(ipi_devolvido["conteudo"]["itens"][0]["imposto_devol"]["vipidevol"], "")
        self.assertEqual(ipi_devolvido["conteudo"]["total"]["vipidevol"], "")
        self.assertFalse(ipi_devolvido["validacao"]["permite_emissao"])

        st_fcp = extrair_contrato_devolucao(rascunho.pk, self.revisor)["icms_st_fcp"]
        self.assertTrue(st_fcp["validacao"]["estrutura_valida"])
        self.assertTrue(st_fcp["validacao"]["origem_completa"])
        self.assertEqual(st_fcp["conteudo"]["itens"][0]["referencia_memoria"]["icms_st"]["valor"], "0.00")
        self.assertEqual(st_fcp["conteudo"]["itens"][0]["hipotese"]["estado"], "NAO_DEFINIDA")
        self.assertEqual(st_fcp["conteudo"]["itens"][0]["destino_fiscal"]["grupo_icms_st"], "")
        ...
        self.assertEqual(obrigatoriedade["validacao"]["quantidade_campos"], 42)
        ...
        self.assertEqual(inventario["validacao"]["quantidade_campos_atomicos"], 115)
    ...
    def test_previa_contrato_somente_leitura_e_acesso_restrito(self):
        ...
        self.assertContains(resposta, "Variantes PIS/COFINS")
        self.assertContains(resposta, "não são inferidas pelo CST")
        ...
        self.assertContains(resposta, "IPI devolvido")
        self.assertContains(resposta, "não é copiado para")
        self.assertContains(resposta, "cEnq")

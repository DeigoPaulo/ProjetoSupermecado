...
def reservar_pedido(*, pedido, usuario, ip=None):
    ...
    if not itens:
        raise ValidationError("Inclua ao menos um item antes de iniciar a separacao.")
    if pedido.tipo_entrega == TipoEntrega.ENTREGA and PoliticaEntrega.objects.filter(filial=pedido.filial, is_active=True).exists() and not pedido.regra_entrega_aplicada:
        raise ValidationError("Calcule a entrega antes de iniciar a separacao.")
    for item in itens:
        movimentar_estoque(
...
def alterar_status_pedido(*, pedido, destino, usuario, ip=None):
    ...
    if destino not in permitidos.get(pedido.status, set()):
        raise ValidationError("Mudanca de status nao permitida para este pedido.")
    if destino == StatusPedido.PRONTO and pedido.itens.exclude(quantidade_separada=models.F("quantidade")).exists():
        raise ValidationError("Conclua a separacao de todos os itens antes de marcar o pedido como pronto.")
...
@transaction.atomic
def registrar_pagamento(*, pedido, forma_pagamento, valor_pago, usuario, ip=None):
    pedido = PedidoOnline.objects.select_for_update().get(pk=pedido.pk)
    if pedido.status in {StatusPedido.CONCLUIDO, StatusPedido.CANCELADO}:
        raise ValidationError("O pagamento deste pedido nao pode mais ser alterado.")
    if valor_pago < pedido.total:
        raise ValidationError("O valor pago nao pode ser menor que o total do pedido.")
    pedido.forma_pagamento = forma_pagamento
    pedido.valor_pago = valor_pago
    pedido.status_pagamento = StatusPagamentoPedido.PAGO
    pedido.pago_em = timezone.now()
    pedido.save(update_fields=["forma_pagamento", "valor_pago", "status_pagamento", "pago_em", "atualizado_em"])
    _log(pedido, usuario, "PAGAMENTO_PEDIDO", f"Pagamento do pedido online {pedido.pk} registrado: {pedido.get_forma_pagamento_display()} - R$ {valor_pago}.", ip)
    return pedido

...
class FinanceiroTests(TestCase):
    ...
    def test_portal_contabilidade_isola_empresa_e_gera_pacote_mensal(self):
        ...
        with ZipFile(BytesIO(pacote.content)) as arquivo:
            ...
            self.assertFalse(manifesto["inventario"]["snapshot_completo"])
            self.assertEqual(manifesto["inventario"]["estado_competencia"], "EM_ANDAMENTO")
            self.assertEqual(manifesto["inventario"]["data_referencia"], timezone.localdate().isoformat())
            self.assertEqual(manifesto["inventario"]["qualidade_temporal"], "SNAPSHOT_IMUTAVEL_PARCIAL_COMPETENCIA")
            self.assertEqual(len(manifesto["inventario"]["fechamentos"]), 1)

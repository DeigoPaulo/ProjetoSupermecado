...
def build_document() -> Document:
    ...
    add_body(doc, "Execute como administrador. O comando seleciona o adaptador ativo com gateway e exibe o IPv4 privado da rede; adaptadores sem rota padrão, como Loopback, VPN, TEF e fiscais, ficam fora da seleção.")
    add_code(doc, "$porta = 8001\n$configRede = Get-NetIPConfiguration | Where-Object { $_.NetAdapter.Status -eq 'Up' -and $_.IPv4DefaultGateway -and $_.IPv4Address } | Select-Object -First 1\n$interface = $configRede.InterfaceAlias\n$ipServidor = $configRede.IPv4Address.IPAddress\nWrite-Host \"Interface: $interface\"\nWrite-Host \"IP do servidor: $ipServidor\"\nSet-NetConnectionProfile -InterfaceAlias $interface -NetworkCategory Private\n$regra = \"DeTec Server - rede privada - porta $porta\"\nGet-NetFirewallRule -DisplayName $regra -ErrorAction SilentlyContinue | Remove-NetFirewallRule\nNew-NetFirewallRule -DisplayName $regra -Direction Inbound -Action Allow -Protocol TCP -LocalPort $porta -Profile Private -RemoteAddress LocalSubnet\nRestart-Service DeigoVarejoServidorLocal\nGet-Service DeigoVarejoServidorLocal\nInvoke-WebRequest \"http://${ipServidor}:$porta/login/\" -UseBasicParsing")
    add_body(doc, "No outro computador, copie o número exibido em IP do servidor. O exemplo abaixo usa 192.168.2.65; substitua-o pelo endereço real da loja:")
    add_code(doc, "$ipServidor = \"192.168.2.65\"\nTest-NetConnection $ipServidor -Port 8001")
    add_callout(doc, "Marcador não é endereço", "Nunca execute literalmente Test-NetConnection IP_DO_SERVIDOR. Esse texto era apenas um marcador; use o IPv4 numérico exibido pelo comando, como 192.168.2.65.", "warning")
    ...
    add_code(doc, "ALLOWED_HOSTS=127.0.0.1,localhost,192.168.2.65,erp.exemplo.com.br  # substitua pelo IPv4 privado real\nCSRF_TRUSTED_ORIGINS=https://erp.exemplo.com.br\nUSE_X_FORWARDED_PROTO=True\nSESSION_COOKIE_SECURE=True\nCSRF_COOKIE_SECURE=True\nSECURE_SSL_REDIRECT=False\n\nRestart-Service DeigoVarejoServidorLocal")
